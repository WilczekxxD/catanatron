...
- might be confusing for the net (after running tests turns out to be very confusing)
...
there are two nodes for every field, one for resource, one for number
field resources changed accordingly to the table below                                       19 inputs
        # 0 - wheat
        # 1 - sheep
        # 2 - wood
        # 3 - brick
...
number of resource cards on hands                                                                3 inputs
a number for every enemy

number of dev cards on hands                                                                     3 inputs  
a number for each enemy 

win conditions per player                                                                         12 inputs
points * 4
road lengths * 4
armies * 4
'''
...
def state_to_vector(state, color_int, edge_list):
    # tile resource vector (19,1)
    tile_resource_vector = tiles_to_resource_vector(state)

    # tile number vector (19, 1)
    tile_number_vector = tiles_to_number_vector(state)
    ...
    # combining them (207, 1)
    vector = tile_resource_vector + tile_number_vector + robber_vector + buildings_vector + ports_vector + roads_vector + hand_vector \
             + played_dev_vector + win_conditions_vector + card_number_vector

    return vector


def tiles_to_resource_vector(state):
    # field
    ...
    for tile in tiles.values():
        ...
        tile_vector = int(tile_resource)
        tile_vectors.append(tile_vector)

    return tile_vectors

def tiles_to_number_vector(state):
    # field
    # this whole section ca be done once a game
    tiles = state.board.map.land_tiles
    tile_vectors = []
